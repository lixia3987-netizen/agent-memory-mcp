...
| pnpm typecheck | 通过 |
| pnpm build | 通过 |
| pnpm test | **58 通过 / 0 失败 / 0 跳过** |
| 最终全套测试耗时 | 约 5.96 秒，仅代表此测试套件 |
...
| contract/http.test.ts | HTTP 显式启用、禁止远程绑定、令牌、Host/Origin、大小限制及真实 SDK 调用 |
...
| integration/providers.test.ts | 真实本地 HTTP 请求、向量索引重排、Key 不落库、重试/超时/熔断、LLM JSON 校验 |
