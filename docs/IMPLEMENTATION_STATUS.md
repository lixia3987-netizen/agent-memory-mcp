...
PR #1 已合并到 main（55c0cb3）。合并后的基线 145 项测试通过；本次按原始需求重新检视实现，修复脱敏后越过正文/标题/metadata 大小上限的问题，新增 4 项策略回归和 1 项启用模型的完整 MCP 工作流，当前本地 150/150 测试通过。完整覆盖矩阵、主分支 CI 结果和可用性结论见 [REVIEW-v0.2.7.md](REVIEW-v0.2.7.md)。0.2.7 不新增依赖、配置或迁移，schema 仍为 105。
