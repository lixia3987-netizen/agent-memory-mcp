...
// System conflict references have their own bounded budget; user attributes keep 16 KiB.
export const relationAttributesSchema = z.object({ conflict_with: z.array(z.uuid()).max(1000).optional() }).catchall(z.unknown()).nullable()
  .refine(v => {
    if (v === null) return true;
    const { conflict_with: _references, ...attributes } = v;
    return Buffer.byteLength(JSON.stringify(attributes)) <= 16384;
  }, 'Attributes exceed 16 KiB');
const aliases = z.array(graphName).max(64).transform(values => {
  const seen=new Set<string>();
  return values.filter(value=>{ const key=canonicalName(value);if (seen.has(key)) return false;seen.add(key);return true; });
});
...
export const relationAddSchema = scopeSchema.extend({ source_entity_id: graphName, predicate: graphName, target_entity_id: graphName,
  attributes: relationAttributesSchema.optional(), confidence: z.number().min(0).max(1).default(1), source_memory_id: graphName.nullable().optional(),
...
export const relationSearchSchema = scopeSchema.extend({ source_entity_id: graphName.optional(), target_entity_id: graphName.optional(), predicate: graphName.optional(), source_memory_id: graphName.optional(),
  at: timestamp.optional(), history: z.boolean().default(false), include_deleted: z.boolean().default(false), include_inactive: z.boolean().default(false), include_stale: z.boolean().optional(),
  limit: z.number().int().min(1).max(100).default(20), offset: z.number().int().min(0).max(100000).default(0) });
export const relationUpdateSchema = scopeSchema.extend({ id: graphName, updates: z.object({ attributes: relationAttributesSchema.optional(), confidence: z.number().min(0).max(1).optional(),
...
export const neighborsSchema = scopeSchema.extend({ entity_id: graphName, at: timestamp.optional(), include_stale: z.boolean().optional(), direction: z.enum(['out', 'in', 'both']).default('both'),
