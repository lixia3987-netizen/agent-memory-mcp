...
const configSchema = z.object({
  homeDir: z.string().min(1), dbPath: z.string().min(1), namespace: z.string().trim().min(1).max(256).default('global'),
  ...
  imports: z.object({ allowedRoots: z.array(z.string().min(1)).default([]), maxFileBytes: z.number().int().positive().max(52428800).default(10485760),
...
export function resolveConfig(cli: ConfigOverrides = {}, env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  if (existsSync(configFile)) {
    try { file = JSON.parse(readFileSync(configFile, 'utf8')) as Record<string, unknown>; }
  ...
  } else if (cli.config || env.AGENT_MEMORY_CONFIG) throw new AppError('NOT_FOUND', 'Explicit configuration file does not exist.');
  ...
  const homeDir = expandPath(String(merged.homeDir ?? initialHome));
  const parsed = configSchema.parse({ ...merged, homeDir, dbPath: expandPath(String(merged.dbPath ?? path.join(homeDir, 'data', 'memory.db'))) });
