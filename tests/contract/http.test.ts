...
test('real MCP HTTP client works and rejects unauthorized, invalid-host and cross-origin requests',async t=>{
  ...
  t.after(()=>{if(previous===undefined)delete process.env.MEMORY_HTTP_TEST_TOKEN;else process.env.MEMORY_HTTP_TEST_TOKEN=previous;});
  const app=await application(t,{http:{enabled:true,port:0,tokenEnv:'MEMORY_HTTP_TEST_TOKEN',maxRequestBytes:4096}});
  const listener=await serveHttp(app);t.after(()=>listener.close());
  ...
  await client.connect(new StreamableHTTPClientTransport(new URL(listener.url),{requestInit:{headers:{Authorization:`Bearer ${token}`}}}));t.after(()=>client.close());
...
test('HTTP receive deadline does not terminate an already received long-running MCP tool call',async t=>{
  ...
  t.after(()=>{if(previous===undefined)delete process.env[tokenEnv];else process.env[tokenEnv]=previous;});
